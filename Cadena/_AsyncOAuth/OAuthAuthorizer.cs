...
namespace AsyncOAuth
{
    ...
    public class OAuthAuthorizer
    {
        ...
        async Task<TokenResponse<T>> GetTokenResponse<T>(string url, OAuthMessageHandler handler, HttpContent postValue,
            Func<string, string, T> tokenFactory) where T : Token
        {
            var client = new HttpClient(handler);

            var content = postValue ?? new FormUrlEncodedContent(Enumerable.Empty<KeyValuePair<string, string>>());
        ...
        public Task<TokenResponse<RequestToken>> GetRequestToken(string requestTokenUrl,
            IEnumerable<KeyValuePair<string, string>> parameters = null, HttpContent postValue = null)
        {
            ...
            var handler = new OAuthMessageHandler(consumerKey, consumerSecret, token: null,
                optionalOAuthHeaderParameters: parameters);
            return GetTokenResponse(requestTokenUrl, handler, postValue,
                (key, secret) => new RequestToken(key, secret));
        }

        /// <summary>asynchronus get GetAccessToken</summary>
        public Task<TokenResponse<AccessToken>> GetAccessToken(string accessTokenUrl, RequestToken requestToken,
            string verifier, IEnumerable<KeyValuePair<string, string>> parameters = null, HttpContent postValue = null)
        {
            ...
            var handler = new OAuthMessageHandler(consumerKey, consumerSecret, token: requestToken,
